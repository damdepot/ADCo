...
specialized sub-agents to inspect the database, recommend optimal configuration knobs,
...
1. `db_inspector` — inspects live database schema, retrieves active configuration knobs, hardware capacity, and synthesizes with application workload patterns from state.
2. `knob_recommender` — synthesizes database and workload findings and formulates tailored configuration knob recommendations.
...
1. **Recommendation**:
   - Delegate to `knob_recommender` to generate configuration knob recommendations based on the database inspection findings and hardware budget.
...
- Delegate to `live_tuner` to apply dynamic knobs to the live database and finalize tuning results.
...
def create_root_agent(model: Union[str, BaseLlm] = "gemini-3.5-flash-lite", buffer_time: float = 0.0) -> LlmAgent:
    ...
    return LlmAgent(
        ...
        tools=[
            AgentTool(create_db_inspector_agent(model, buffer_time=buffer_time)),
            AgentTool(create_knob_recommender_agent(model, buffer_time=buffer_time)),

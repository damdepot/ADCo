...
# ===========================================================================
# 3. apply_knobs_staging Tool Tests
# ===========================================================================
...
@patch("src.knob_tuner.sub_agents.knob_checker.tools.get_container_host_port")
@patch("src.knob_tuner.sub_agents.knob_checker.tools.restart_docker_db")
def test_restart_database_staging_success(mock_restart, mock_get_port, mock_db_config_pg):
    ...
    mock_get_port.assert_called_once_with("my-staging-container", 5432)
    assert tc.state["staging_db_config"].port == 54321

@patch("src.knob_tuner.sub_agents.knob_checker.tools.get_container_host_port")
@patch("src.knob_tuner.sub_agents.knob_checker.tools.restart_docker_db")
def test_restart_database_staging_port_fallback(mock_restart, mock_get_port, mock_db_config_pg):
    mock_restart.return_value = (True, "Container restarted")
    mock_get_port.side_effect = Exception("Docker port command failed")
    original_port = mock_db_config_pg.port
    tc = MockToolContext({
        "staging_db_config": mock_db_config_pg,
        "staging_docker_container": "my-staging-container"
    })
    ...
    mock_restart.assert_called_once_with("my-staging-container", db_type=mock_db_config_pg.db_type)
    mock_get_port.assert_called_once_with("my-staging-container", 5432)
    assert tc.state["staging_db_config"].port == original_port
...
@patch("src.knob_tuner.sub_agents.knob_checker.tools.start_staging_db")
@patch("src.knob_tuner.sub_agents.knob_checker.tools.is_docker_available", return_value=(True, "Docker is ready"))
def test_setup_staging_docker_database_from_db_name(mock_docker_avail, mock_start_staging, mock_db_config_pg):
    ...
    mock_start_staging.assert_called_once()
    _, kwargs = mock_start_staging.call_args
    assert kwargs["database"] == "from_db_name"
...
def test_cleanup_staging_docker_success(mock_stop_staging, mock_db_config_pg):
    tc = MockToolContext({
        "staging_docker_container": "adco-staging-pg-cleanup",
        "staging_db_config": mock_db_config_pg,
    })
    ...
    assert tc.state.get("staging_db_config") is None
    assert tc.state["staging_db_config"] is None
...
def test_cleanup_staging_docker_warning(mock_stop_staging, mock_db_config_pg):
    tc = MockToolContext({
        "staging_docker_container": "adco-staging-pg-cleanup",
        "staging_db_config": mock_db_config_pg,
    })
...
def test_recreate_database_staging_missing_config_fallback(mock_setup):
    mock_setup.return_value = "## Staging Docker Container Initialized: **OK**"
    tc = MockToolContext({"staging_docker_container": "old-container"})
...
@patch("src.knob_tuner.sub_agents.knob_checker.tools.setup_staging_docker")
def test_recreate_database_staging_missing_both_fallback(mock_setup):
    mock_setup.return_value = "## Staging Docker Container Initialized: **OK**"
    tc = MockToolContext({})
